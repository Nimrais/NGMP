...
Results for direct-backend RxInfer Softplus MLPs with vector transport, trained
...
|---|---|---:|---|
| RxInfer flattened Softplus MLP | 32 | 50 | `alpha=0.2`, direct weight-site scale 0.5 |
| RxInfer two-hidden-layer Softplus MLP | 32 / 32 | 50 | 3 inner site-update iterations; direct weight-site scale 1.0 |
...
accuracy. Among the RxInfer models, the one-hidden-layer model achieved the
stronger result: 92.3% validation accuracy and 91.3% test accuracy. The
two-hidden-layer RxInfer model reached 90.7% validation accuracy at epoch 47
and 89.7% test accuracy.
