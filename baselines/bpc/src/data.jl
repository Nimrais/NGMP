...
function outer_and_inner_splits(dataset, split_id::Int, config::BPCConfig)
    spec = uci_regression_split(
        size(dataset.features, 1), split_id;
    ...
    return (
        outer = prepare_uci_regression_partition(
            ...
            spec.train_indices, spec.test_indices,
        ),
        inner = prepare_uci_regression_partition(
            dataset.features, dataset.targets,
            spec.inner_train_indices, spec.validation_indices,
        ),
