# =============================================================================
# Validation: cavi_inner.jl vs RxInfer `infer` on inner_exp_ensemble
# =============================================================================
# Runs both on identical synthetic inputs and reports max |Δ| on all posterior
# statistics (E[w], Σ_w, E[z], V[z], shape/rate of τ) at several iteration
# counts and for several within-sweep schedules, plus wall-clock timings.
# =============================================================================
...
function compare(res, cavi)
    ...
    db = maximum(abs(rate(qτ[i]) / rate(cavi.τ[i]) - 1) for i in 1:nf)  # relative, rates are large
    return dμw, dΣw, dmz, dvz, da, db
end

schedules = ((:z, :w, :τ), (:w, :z, :τ), (:z, :τ, :w), (:τ, :z, :w))

println("== posterior agreement (max |Δ|; Δrate is relative) ==")
@printf("%5s  %-12s  %10s %10s %10s %10s %8s %10s\n",
        "iters", "schedule", "E[w]", "cov(w)", "E[z]", "V[z]", "shape(τ)", "rate(τ)")
for iters in (1, 2, 5, 50)
    res = run_rxinfer(iters)
    for sch in schedules
        cavi = cavi_inner_infer(features, obsz, Rz, w0, τ0; iterations = iters, schedule = sch)
        d = compare(res, cavi)
        @printf("%5d  %-12s  %10.2e %10.2e %10.2e %10.2e %8.1e %10.2e\n",
                iters, join(String.(sch), ""), d...)
    end
    println()
end

println("== timing (iterations = 5) ==")
run_rxinfer(2)                                    # compile
t_rx = @elapsed run_rxinfer(5)
cavi_inner_infer(features, obsz, Rz, w0, τ0; iterations = 2)   # compile
t_cv = @elapsed cavi_inner_infer(features, obsz, Rz, w0, τ0; iterations = 5)
@printf("no=%d:  RxInfer %.3f s   CAVI %.5f s   speedup %.0f×\n", no, t_rx, t_cv, t_rx / t_cv)

no_full = 3398
features_f = [randn(rng, nfeat) for _ in 1:no_full]
obsz_f = randn(MersenneTwister(3), nf, no_full) .* 0.5 .+ 2.0
Rz_f = fill(0.7, nf, no_full)
cavi_inner_infer(features_f, obsz_f, Rz_f, w0, τ0; iterations = 2)
t_cvf = @elapsed cavi_inner_infer(features_f, obsz_f, Rz_f, w0, τ0; iterations = 5)
@printf("no=%d (full scale): CAVI %.4f s per outer  →  40 outers ≈ %.1f s (vs 10:44 measured with RxInfer)\n",
        no_full, t_cvf, 40 * t_cvf)
