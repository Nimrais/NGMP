...
# Provenance: Codex scratch experiment (2026-07-29), originally at
# /private/tmp/why_hierarchy_deep_kernel_relaxed.jl. Validates (a) the relaxed
# constraint q(w, score, precision) — runs end-to-end, needs an extra mu(w)
# message initialization, does NOT remove the depth-SNR bottleneck — and
# (b) prototype softdot(:y)/(:x) NaturalGradientMessage cavity rules using
# GaussianStudentTMessage with a rank-one lift onto the weight edge. The rules
# were productionized into src/nodes/softdot/rules/natural_gradient.jl.
...
begin
    using Pkg
    Pkg.activate("/Users/mykola/repos/surrogate-modelling")
end
...
# Exact-BP log messages for a fully clustered carrier softdot, tangent-projected
# back to Gaussian/Gamma edge families.
@rule softdot(:y, NaturalGradientMessage) (
    ...
    exact = Logpdf(GaussianStudentTMessage(
        dot(f, mx), dot(f, Vx * f), shape(m_γ), rate(m_γ),
    ))
    site = project(resolve_projection(getprojection(vconstraint)), q_y, exact)
    NaturalGradientMP.apply_damping!(meta, site)
end
...
@rule softdot(:x, NaturalGradientMessage) (
    ...
    exact = Logpdf(GaussianStudentTMessage(
        my, vy, shape(m_γ), rate(m_γ),
    ))
    scalar_site = project(
        resolve_projection(getprojection(vconstraint)), q_residual, exact,
    )
    scalar_natural = getnaturalparameters(scalar_site)
    vector_site = MvNormalWeightedMeanPrecision(
        scalar_natural[1] .* f,
        (-2 * scalar_natural[2]) .* (f * f'),
    )
    NaturalGradientMP.apply_damping!(meta, vector_site)
end
...
begin
    ...
    @constraints function hierarchy_constraints()
        q(v, w, score, precision, y) = q(v, y)q(w, score, precision)
        q(v)::MomentForm()
        q(w)::MomentForm()
    end
end
