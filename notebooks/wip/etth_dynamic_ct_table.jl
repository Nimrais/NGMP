# ETTh1/ETTh2 x {96, 192, 336, 720}: the linear-66d 2-level arm against the paper's
# Dynamic ensemble, scored with the paper's own MSE/NLL confidence-interval recipe.
#
# The 2x2 in experiments/dynamic_deep_kernel_precision.jl measured a single cell
# (ETTh1 h96) and found this arm ahead of the published Dynamic row on both MSE and
# logpdf. One cell is not a result; this fills all eight.
#
...
const ARM = :linear_shrunk          # linear 66d, 2 levels, shrunk + anchored prior
...
function fit_cell(dataset, horizon)
    ...
    started = time()
    arm = fit_named_arm(ARM, config; train_features, targets, predictions)
...
function console_table(io = stdout)
    println(io, "\n", "="^108)
    println(io, "Dynamic ensemble on standardized OT: CT arm (linear 66d, 2 levels) vs " *
