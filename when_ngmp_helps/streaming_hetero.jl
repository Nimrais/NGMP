...
const STREAMING_ARMS = (
    (key = "vmp", method = "VMP", iterations = 240),
    (key = "vmp1", method = "VMP", iterations = 1),
    (key = "ngmp", method = "NGMP-cavity", iterations = 240),
)

const ARM_STYLES = Dict(
    "vmp" => (color = COLORS.vmp, linestyle = :dash),
    "vmp1" => (color = COLORS.vmp1, linestyle = :dot),
...
function render()
    ...
    preview = plot(
        plot(panels[("vmp", "full")]; title = "$(method_label("vmp")), full batch"),
        plot(panels[("vmp", "sequential")]; title = "$(method_label("vmp")), sequential"),
